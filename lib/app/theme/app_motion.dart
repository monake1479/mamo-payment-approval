...
class AppPageTransitionSwitcher extends StatefulWidget {
  ...
  const AppPageTransitionSwitcher.indexed({
...
class _AppPageTransitionSwitcherState extends State<AppPageTransitionSwitcher>
  ...
  void initState() {
    ...
    assert(widget.children != null || widget.child!.key != null);
  }
  ...
  void didChangeDependencies() {
    ...
    if (_animationsDisabled &&
        (_outgoingChild != null || _outgoingIndex != null)) {
      _controller.value = 1;
      _outgoingChild = null;
      _outgoingIndex = null;
    }
  ...
  Widget _buildIndexed(BuildContext context) {
    ...
    final int? outgoingIndex = _outgoingIndex;
    final double textDirection = Directionality.of(context) == TextDirection.ltr
        ? 1
        : -1;
    final double offset =
    ...
    return ClipRect(
      child: AnimatedBuilder(
        animation: _controller,
        builder: (BuildContext context, Widget? child) {
          final double progress = _animationsDisabled || outgoingIndex == null
              ? 1
              : AppMotion.pageCurve.transform(_controller.value);
          return Stack(
            fit: StackFit.expand,
            children: List<Widget>.generate(children.length, (int index) {
              final bool current = index == _currentIndex;
              final bool outgoing = index == outgoingIndex;
              final Widget retainedChild = TickerMode(
                enabled: current,
                child: IgnorePointer(
                  ignoring: !current,
                  child: ColoredBox(
              ...
              if (!current && !outgoing) {
                return Offstage(child: retainedChild);
              }
              final double dx = current
                  ? offset * (1 - progress)
                  : -offset * progress;
