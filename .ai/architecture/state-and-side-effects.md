...
- Page-scoped state owners are provided by the page itself: annotate the BLoC/Cubit with `@injectable` (a factory) and let the page widget wrap its body in `BlocProvider(create: (_) => getIt<XyzBloc>())`. The router, `lib/app/di/`, and the app shell must not wrap page widgets in providers, take "create the bloc" callbacks, or otherwise know which state owners a page needs (`PaymentsPage` and `PaymentsSearchBloc` are the reference). Only process-wide owners such as `PaymentsCubit` and `ThemeModeCubit` are created in composition and passed down. Widget tests register the factory in `getIt` (see `registerPaymentsSearchBloc` in `test/support/payments_test_support.dart`) instead of threading blocs through constructors.
