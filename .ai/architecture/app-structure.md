...
- Keep the mock-backend boundary transport-shaped: raw request/response values and backend exceptions cross into data sources. Do not let mock implementations return application DTOs, domain models, `Result`, or `PaymentsFailure`, because that would bypass the same mapping required with a real API.
