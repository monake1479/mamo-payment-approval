...
- Annotate shared data sources, repositories, and use cases with `@lazySingleton`; generated registration is owned by `lib/app/di/`. Do not resolve `getIt` inside business methods. Annotate page-scoped BLoCs/Cubits with `@injectable` and resolve them only in the page's own `BlocProvider(create:)`; resolving or constructing a page's state owner in the router or in `lib/app/di/` is an anti-pattern because the router would then have to know what every page injects below it.
...
- Prefer constructor injection and explicit composition in `lib/app/`. `get_it` is in use (ADR 0011): resolve dependencies in composition for process-wide owners and in a page's own `BlocProvider(create:)` for page-scoped owners, never inside business methods and never in route builders.
