...
- A new feature state owner is `@injectable` and provided through `BlocProvider(create: (_) => getIt<...>())` as close as possible to the subtree that uses it, lifted to the top of the page only when several parts of that page share it; do not add provider wiring or factory callbacks to `MamoPaymentRouter` or `configure_dependencies.dart`. `lib/features/payments/pages/payments_page.dart` with `PaymentsSearchBloc` is the reference; widget tests register the factory in `getIt` through the test support helpers.
...
- [Foundation page](../../../lib/features/payments/pages/foundation_page.dart)
