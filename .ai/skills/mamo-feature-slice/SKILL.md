...
- A new page-scoped state owner is `@injectable` and provided by its page through `BlocProvider(create: (_) => getIt<...>())`; do not add provider wiring or factory callbacks to `MamoPaymentRouter` or `configure_dependencies.dart`. `lib/features/payments/pages/payments_page.dart` with `PaymentsSearchBloc` is the reference; widget tests register the factory in `getIt` through the test support helpers.
