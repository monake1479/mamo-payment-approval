...
void main() {
    ...
    for (final Size size in <Size>[
      ...
      testWidgets('renders complete decided details at $size in $brightness', (
        ...
        final StubPaymentsRepository repository = StubPaymentsRepository(
          onLoad: () async =>
              PaymentsSuccess<List<Payment>>(<Payment>[payment]),
        );
  ...
  testWidgets('resolves details from the latest canonical collection state', (
    ...
    final StubPaymentsRepository repository = StubPaymentsRepository(
      onLoad: () async =>
          PaymentsSuccess<List<Payment>>(<Payment>[currentPayment]),
    );
    final PaymentsCubit cubit = createPaymentsCubit(repository);
    addTearDown(cubit.close);
    await cubit.load();
  ...
  testWidgets('invalid and pending identifiers render safe missing UI', (
    ...
    final StubPaymentsRepository repository = StubPaymentsRepository(
      onLoad: () async => PaymentsSuccess<List<Payment>>(<Payment>[pending]),
    );
    final PaymentsCubit cubit = createPaymentsCubit(repository);
    addTearDown(cubit.close);
    await cubit.load();
