...
Amounts remain positive finite Dart `double` values with at most two decimal places at the DTO boundary. The application does not define a maximum transaction or aggregate amount; those limits belong to an account configuration or authoritative backend if one is introduced. There is no precision tolerance, integer-fils shadow model, or special money wrapper. Wire amounts round-trip through `PaymentAmountJsonConverter`; exactly two decimal places are a presentation concern.

## Boundaries and exclusions

The mock backend is session-only and resets to deterministic seed data after process termination. Its failure interval is disabled in normal composition and can be enabled explicitly in tests; uncontrolled randomness is not used. Test-only clock, records, latency, currency, and failure cadence belong to `MockPaymentsBackend`, so the production-shaped data source has one dependency-only constructor and no `@ignoreParam` configuration. No deployed backend, database, navigation, screen, authentication adapter, or approval-disclosure state is introduced here.
