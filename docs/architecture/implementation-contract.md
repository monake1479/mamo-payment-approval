...
- Payment `amount` is a positive finite Dart `double` with at most two decimal places at the DTO boundary. Do not impose a client-side transaction or aggregate maximum; account/backend limits belong to their authoritative configuration when introduced.
...
- Serialized demo records use an amount string such as `"1234.56"`, an explicit currency code such as `"AED"`, and ISO 8601 UTC timestamps ending in `Z`. No real HTTP API is introduced. DTO parsing and validation are independent of device locale.
