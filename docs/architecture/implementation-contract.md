...
PaymentsCubit is the single collection state owner. Its dedicated `states/payments/` directory contains `payments_cubit.dart` and `payments_state.dart`. It owns UI progress, stale-completion guards, and canonical state emission, while injected use cases validate inputs, repository results, and business transitions. The Cubit updates the collection before returning operation success. UI and details derive from this state. The short-lived ApprovalCubit owns only authentication, disclosure, and submission state for one request and receives the sibling `states/approval/` directory. Inject the authentication use cases and a typed decision callback/operation; it must not subscribe to another Cubit or maintain a second collection. App composition connects the callback to the authoritative write path.

The authentication task can implement its data contract independently: `LocalAuthenticationUseCase.call()` returns a typed success/cancelled/unavailable/failed outcome, `StopLocalAuthenticationUseCase.call()` exposes cancellation, and `IsLocalAuthSupportedUseCase.call()` reports capability. The use cases delegate to `LocalAuthRepository`, which owns the cancellation state machine and calls the `LocalAuthClient` plugin seam. Presentation supplies localized prompt text through composition, not domain imports. Cancellation or late completion never grants disclosure. Platform plugins live only behind the client seam.
