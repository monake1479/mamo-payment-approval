...
delegated routine selections remain subject to owner review. Independent review,
current-head CI, native builds, and critical journeys remain delivery gates rather
than being implied by code integration.
...
| 1. Read-only payments | Model, deterministic async repository, collection state, decided-payment list; `PAY-01/02`, `MONEY-01` | Implemented with accepted ADR 0002 semantics; AED-only, pending excluded from history; failures, ordering, loading/empty/error, compact/expanded tests; Maestro list flow |
...
| 3. Incoming request and rejection | Draggable global action, masked non-dismissible overlay, rejection, canonical update; `DEBUG-01..04`, `APPROVAL-01..03/05/07..09`, rejection part of `PAY-04` | Implemented with one active request, no queue/replacement, accepted masks/tie-breaker, and dismissal/drag/origin/duplicate coverage |
...
| 6. Delivery and reviewer guide | Installable Android APK, supported/tested iOS, critical journeys; `DELIVERY-01/02` | Private Actions artifact workflow implemented; native builds, artifact access, source/hash provenance, journeys, and evidence limitations remain final integration checks |
...
Compact layouts use bottom navigation; expanded layouts use a rail. Loading, empty,
typed-error/retry, success, light/dark, account-zone dates, fixed AED formatting,
and 200% text are covered by widget tests. `PaymentFlowLayer` now composes the
implemented global approval overlay and draggable action above the router.
`maestro/payments_list.yaml` and `maestro/payment_details.yaml` cover the
deterministic list, summary, details, and return journeys; native run evidence
remains separate from Flutter tests.
...
use the delegated `FLAG_SECURE` choice on API 24–32, which also blocks ordinary
foreground screenshots/recording; API 33+ uses the recents-only boundary. PIN/expiry
remain deferred. Track native device evidence independently from bootstrap tests.
...
redirect. `PaymentFlowLayer` is the concrete app-level owner of the approval route,
draggable action, and their lifecycle/navigation effects; there is no generic layer
extension seam.

## Localization foundation increment

English ARB messages and generated `AppLocalizations` cover the application shell,
payment screens, approval states, errors, actions, and semantics (ADR 0008).
English/regional-English resolution, unsupported-locale fallback,
compact/expanded layouts, large text, generation, and native compilation are
verified separately. Fixed AED formatting is a domain/presentation contract rather
than a locale-selected currency format. Native flavor launcher names remain
platform metadata. CI generates localization output before analysis/tests.
...
masked-overlay rejection, and debug-drag/session flows. There are no Maestro CI jobs
yet; retain local results and tool-version provenance.

Native-authentication success, cancellation, unavailability, and lifecycle checks
remain separate native evidence because the app has no automation bypass. Label
simulator biometric input as simulated and do not present it as physical-device
verification. Future CI automation may upload native reports when runner access is
available; no paid cloud service is implied.
...
## Remaining decisions and historical gates
...
- ADR 0002 and the implementation contract resolve comparison, summation,
  validation, and serialization while preserving the `double` domain amount.
- Deterministic fixtures explicitly include pending items and prove their exclusion
  from history; approved-only membership uses decision time and `Asia/Dubai`.
- The overlay masks and stable ordering tie-breaker are implemented. Process
  termination resets session state. A submitted decision may complete once in the
  background, with its terminal UI effect consumed once after resume.
...
- Before promotion: verify private artifact access, final source/binary provenance,
  Android/iOS journeys, and honest native-authentication limits. iOS distribution is
  not required; iOS support and native verification remain required.
