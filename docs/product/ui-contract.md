...
| Payments | All approved/rejected history newest decision first. Each tappable row contains party, fixed-format amount, status text/icon, and decision date. Horizontal swipe right opens Home. Reaching Payments from Home—through the View all action, a navigation control, or a swipe—pushes no route, so system Back (gesture or button) returns to Home rather than leaving the app; the app only exits from the Home start destination. Once the history has loaded, one scroll view below the reporting-zone context holds an outlined search field (search icon, clear-text action) and a wrapping row of controls, left to right: a status dropdown chip (all, approved, rejected), a decision-date chip that opens the Material date-range picker and shows the chosen days with a remove action, a sort dropdown chip, and, only while a criterion is active, a Clear text button. Controls wrap onto further lines rather than scrolling horizontally and scroll away with the list. The whole area supports pull-to-refresh. No pending list or pagination in baseline. |
...
- Search (extension slice): with no query, no selected status, no date window, and the default order, the full history and its empty state are unchanged. Clear removes every criterion and empties the field; the field's own icon clears only the text. Sort options: newest first (default), oldest first, highest amount, lowest amount, counterparty A to Z; any non-default order or date window is an active criterion and shows the backend-ordered result. Pull-to-refresh on the list, empty, and error areas reloads the collection and re-runs an open search. Text matches the counterparty and reference only; the pending request never appears in a result. Debounced typing shows a labelled search progress state, then either the result count above the same tappable rows, a search-specific empty explanation, or a search-specific error with retry. Clearing returns to the full history. The search is page-scoped session state: it re-runs when the collection changes and survives the Home/Payments switch and system Back, and it is not a pushed route.
