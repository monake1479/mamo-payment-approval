...
`AppTheme` owns light/dark Material themes, spacing, radii, touch targets, layout thresholds, outlined field states, chips, navigation surfaces, dividers, and fixed snackbars. A user-selectable appearance mode — System, Light, or Dark, defaulting to System and persisted across launches — drives the theme (`UI-03`; see [ADR 0013](../decisions/0013-persistent-theme-mode.md)). Normal app composition applies the selection; the standalone failure UI continues to follow the system appearance. Both normal app composition and standalone failure UI support both appearances. Use system typography via `TextTheme`. Payment status uses the shared `PaymentStatusChip`, which combines a localized word and icon so meaning never depends on colour alone. Rejected uses the Material error palette, approved uses primary tones, and pending uses a contrasting amber container in both appearances. Informational feedback uses a dedicated blue container so it remains distinct from primary-toned success feedback.
...
| Payments | All approved/rejected history newest decision first. Each tappable row contains party, fixed-format amount, status text/icon, and decision date. Horizontal swipe right opens Home. Reaching Payments from Home—through the View all action, a navigation control, or a swipe—pushes no route, so system Back (gesture or button) returns to Home rather than leaving the app; the app only exits from the Home start destination. No pending list, search, filters, or pagination in baseline. |
...
| Settings | Reached from a labelled action in the Home heading, pushed as a full-screen route above the navigation shell with an in-app Back. Hosts the appearance-mode chooser: a mutually exclusive System/Light/Dark control whose selected option is shown by an indicator icon and accessibility state, not colour alone. Each option is a full-width target that stays above the minimum touch size at 200% text. Below it, an About section shows a details card (Version, Build, Environment, Package, Device authentication as labelled selectable rows; a live-region loading row; or a failure message with a Try again action) and a static card describing the app, what is included, and known limitations. Availability is conveyed by text and icon, not colour alone. |
